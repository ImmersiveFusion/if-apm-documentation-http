# What's New

Recent updates and features in IAPM. For complete version history, see the release notes for [IAPM Desktop](../Products/IAPM-Desktop/release-notes.md) and [IAPM Web](../Products/IAPM-Web/release-notes.md).

---

## Latest Highlights

### AI Copilot (Desktop 1.11)

Interact with your telemetry using natural language. Ask questions via chat or voice - no queries required.

| Feature | Description |
|---------|-------------|
| **Chat interface** | Ask questions about your traces and metrics |
| **Voice commands** | Hands-free exploration with Tessa AI |
| **Always-listening mode** | Say "Hey Tessa" to activate |

### Span View & Network Graph (Web 3.71)

New visualization options for trace analysis:

| View | Best For |
|------|----------|
| **Span View** | Waterfall timeline showing span relationships |
| **Network Graph** | Interactive service dependency visualization |
| **Nested Tags** | Collapsible tree view for hierarchical data |

### macOS Support (Desktop 1.11)

Experimental support for Apple Silicon Macs. Download and test on macOS while we work toward full feature parity.

---

## Recent Updates

### December 2025

- **Web 3.71**: Major dashboard overhaul, Angular 21 upgrade
- **Web 3.71**: Dark/light theme improvements across all components
- **Web 3.71**: Renamed to IAPM throughout application

### April 2025

- **Desktop 1.8**: Hall of Supporters with Buzludzha Monument
- **Desktop 1.8**: .NET 9.0 rollup

### January 2025

- **Desktop 1.6**: Live Charts showing Published/Processed/Failed/Lost counts
- **Desktop 1.6**: Top view with automatic zoom
- **Desktop 1.6**: Improved OTLP exporter delay handling

### December 2024

- **Desktop 1.5**: Beta VR functionality
- **Desktop 1.5**: Topology graph improvements

---

## Coming Soon

| Feature | Product | Status |
|---------|---------|--------|
| Filters UI | Desktop | In development |
| Full macOS support | Desktop | Experimental |
| Replay functionality | Desktop | In development |

---

## Feature Requests

Have an idea for IAPM? We'd love to hear from you:

- [Discord community](https://discord.gg/zevywnQp6K){ target="_blank" } - Share ideas and vote on features
- [Email feedback](mailto:feedback@immersivefusion.com) - Direct feedback to our team

---

## Stay Updated

| Channel | Link |
|---------|------|
| Release notes (Desktop) | [View releases](../Products/IAPM-Desktop/release-notes.md) |
| Release notes (Web) | [View releases](../Products/IAPM-Web/release-notes.md) |
| YouTube | [Tutorials & demos](https://www.youtube.com/@immersivefusion){ target="_blank" } |
| LinkedIn | [Company updates](https://www.linkedin.com/company/immersivefusion){ target="_blank" } |
